...
plotly_advanced_blue_print = flask.Blueprint(
    name="plotlyadvanced", import_name=__name__)
...
def render_country_population(df: pd.DataFrame, country: str) -> str:
    logging.info(
        f"Inside function render_plotly_graph, selected country={country}, df={df}")
    ...
    if country is None:
        ...
        logging.info(
            f"No country was specified, therefore going to use '{country}'")
    ...
    html_helper = lib.SelectElementHelper(
        values=countries, labels=countries, selected_value=country)
    return flask.render_template('plotlyadvanced.html', graphJSON=graph_json, helper=html_helper)
